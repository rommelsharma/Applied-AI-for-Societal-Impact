...
def build_mlops(doc: Document) -> None:
    """MLOps best practices and operational concerns."""
    add_heading(doc, "9. MLOps Best Practices", level=1)

    add_heading(doc, "9.1 Reproducibility", level=2)
    add_bullets(
        doc,
        [
            "Pinned dependency floors in ``requirements.txt`` (boto3, faiss-cpu, numpy, PyMuPDF, python-dotenv).",
            "Centralised ``BEDROCK_SETTINGS`` so model ids and embedding dimensions are versioned in ``.env``.",
            "Deterministic chunking parameters (``CHUNK_SIZE``, ``CHUNK_OVERLAP``) declared as module-level constants.",
            "Frozen taxonomy and support-concept JSON files in ``data/metadata/`` so the symbolic layer is auditable.",
            "Vector-store ``manifest.json`` records corpus count, dimension, and index type at build time.",
        ],
    )

    add_heading(doc, "9.2 CI/CD Strategy", level=2)
    add_bullets(
        doc,
        [
            "GitHub Actions pipeline: lint -> unit tests -> taxonomy schema validation -> ingestion smoke run -> container build -> push to ECR.",
            "Blue/green deploy via ECS service-update or Lambda alias-shift on traffic.",
            "Manual approval gate before production deploys until automated evaluation gating is in place.",
        ],
    )
    ...
    add_bullets(
        doc,
        [
            "Every chunk carries source, file_name, author, title, publication_year, parser_notes, cleaning_notes, and chunk_index from ingestion through retrieval through final response payload.",
            "Vector index manifest records the index type and corpus dimension so the runtime can refuse to start on a mismatch.",
            "Bedrock model id is stamped into observability metadata for every inference call so behaviour changes after a model upgrade are traceable.",
        ],
    )

    add_heading(doc, "9.4 Observability", level=2)
    add_bullets(
        doc,
        [
            "Structured logs (JSON) for every retrieval and inference call: scenario id, top-k, retrieved chunk ids, similarity scores, token usage.",
            "CloudWatch metrics: P50/P95 retrieval latency, P50/P95 inference latency, JSON-parse retry rate, fallback-retrieval rate.",
            "Distributed tracing via AWS X-Ray spans for retrieve, format-context, baseline-call, RAG-call.",
            "Alarms on Bedrock throttling, high JSON-retry rate, and elevated empty-result rate.",
        ],
    )
    ...
    add_paragraph(
        doc,
        "Beyond unit tests, the deployment pipeline should run the evaluation harness "
        "against the canonical scenario set on every release candidate. Releases are "
        "blocked when:",
    )
    add_bullets(
        doc,
        [
            ...
            "Bias-count delta between baseline and RAG indicates RAG-induced regression.",
            "P95 inference latency exceeds the agreed SLO.",
        ],
    )

    add_heading(doc, "9.6 Responsible AI and Governance", level=2)
    add_bullets(
        doc,
        [
            "Disclaimer surfaced to every user: not legal/medical/HR advice.",
            "Strict closed-vocabulary output prevents the model from inventing biases.",
            "Source attribution forces the model to cite chunks rather than confabulate.",
            "Human-in-the-loop is the explicit operating model; the system never automates a decision.",
            "Audit logs preserve scenarios, retrieved chunks, and model outputs for review under data-handling controls.",
        ],
    )
...
def build_risks(doc: Document) -> None:
    ...
    add_heading(doc, "10. Risks, Limitations, and Mitigations", level=1)
...
def build_roadmap(doc: Document) -> None:
    ...
    add_heading(doc, "11. Roadmap and Next Steps", level=1)

    add_heading(doc, "11.1 Short Term", level=2)
    add_bullets(
        doc,
        [
    ...
    add_heading(doc, "11.2 Medium Term", level=2)
    add_bullets(
        doc,
        [
    ...
    add_heading(doc, "11.3 Long Term", level=2)
    add_bullets(
        doc,
        [
...
def build_appendix(doc: Document) -> None:
    ...
    add_heading(doc, "12.2 Primary Source Books in the Corpus", level=2)
    add_bullets(
        doc,
        [
    ...
    add_heading(doc, "12.3 References", level=2)
    add_bullets(
        doc,
        [
